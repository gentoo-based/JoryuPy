...
class Moderation(commands.Cog):
    ...
    async def timeout(self, ctx: commands.Context, user: Member, time: str, reason: Optional[str]):
        if "s" in time:
            await user.timeout(timedelta(seconds=float(time.split("s")[0])), reason=reason)
        elif "m" in time:
            await user.timeout(timedelta(minutes=float(time.split("m")[0])), reason=reason)
        elif "h" in time:
            await user.timeout(timedelta(hours=float(time.split("h")[0])), reason=reason)
        elif "d" in time:
    ...
    async def warn(self, ctx: commands.Context, user: Member, reason: Optional[str]):
        awarns = await execute_query("SELECT * FROM warnings WHERE guild_id = ? AND user_id = ?", (ctx.guild.id, user.id))
        warns = int(awarns[-1][3]) + 1 if awarns else 1
        await execute_query("INSERT INTO warnings (guild_id, user_id, warns, reason, moderator_id) VALUES (?, ?, ?, ?, ?)", (ctx.guild.id, user.id, warns, reason, ctx.author.id))
    ...
    async def setprefix(self, ctx: commands.Context, prefix: str):
        try:
            await execute_query("INSERT INTO prefixes (guild_id, prefix) VALUES (?, ?)", (ctx.guild.id, prefix))
            prefix = await execute_query("SELECT prefix FROM prefixes WHERE guild_id = ?", (ctx.guild.id,))
            await ctx.send(f"Successfully set the prefix for the guild to `{prefix[0]}`")
