...
async def on_ready():
    ...
    await joryu.load_extension("misc")
    await joryu.load_extension("moderation")
    await joryu.load_extension("owner")
    await joryu.tree.sync()
    print(f"{joryu.user.name}#{joryu.user.discriminator} has successfully entered the Discord API Gateway with {joryu.shard_count} Shards.")
...
async def on_shard_ready(shard_id):
    while True:
        ...
        await joryu.change_presence(activity=randomizedActivity, status=Status.online, shard_id=shard_id)
